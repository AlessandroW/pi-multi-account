...
- **Deduplicates provably identical accounts** so duplicate Codex `accountId` plus workspace-plan/email identities and identical credentials do not consume multiple rotation slots or get separate cooldowns. Personal and business Codex workspaces can coexist when their access tokens expose distinct plan types, and different-email Codex logins remain separate even when OpenAI gives them the same account id; new provable duplicate logins are rejected before the redundant slot is saved.
...
- **Duplicate identities** share one rotation position and one cooldown, and status/startup identifies the redundant slot. Codex/ChatGPT is matched by stored `accountId`; when its JWT exposes `chatgpt_plan_type` or the standard OIDC profile email, those claims also distinguish separate workspaces/logins. Identical API keys or literal identical tokens are also matched. Separate Anthropic OAuth logins cannot be proven identical because Anthropic's stored credential exposes no stable account identifier.
