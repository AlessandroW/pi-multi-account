...
- **Different-email Codex logins are no longer rejected as duplicates.** OpenAI can reuse an account id across login or workspace contexts, so duplicate detection now also compares hashed plan and OIDC email claims when available; missing claims remain conservative and still deduplicate by account id.
